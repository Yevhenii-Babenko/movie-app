...
export const mutations: MutationTree<RootState> = {
  ...
  UPDATASEARCHEDMOVIES: (state, payload: []) =>
...
export const actions: ActionTree<RootState, RootState> = {
  ...
  async fetctSearchedMovies({ commit }, input) {
    try {
      commit('SETLOADED', true);
      const searchedMovies = await this.$axios
        .$get(
          `search/movie?api_key=84a698300a40f7d90c5505eebd96b53b&language=en-US&page=1&query=${input}`
        )
        .then((data) => data.results);
  ...
  async fetchSingleMovieById({ commit }, id: number) {
    try {
      commit('SETLOADED', true);
      const getMovie = await this.$axios
        .$get(
          `movie/${id}?api_key=84a698300a40f7d90c5505eebd96b53b&language=en-US`
        )
        .then((data) => data);
